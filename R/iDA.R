...
iDA_core <- function(data.use,
  ...
  snn <- getSNN(data.use = transformed, set.seed = set.seed, k.param = k.param,
                prune.SNN = prune.SNN)
  ...
  for (i in 2:15) {
    modularity <- c(modularity,
                    modularity(snn, suppressWarnings(igraph::cut_at(walktrapClusters, n = i))))
  }
  ...
  while (concordance < 0.98) {
    ...
    for (j in 2:15) {
      modularity <- c(modularity, modularity(snn_transformed,
                                             suppressWarnings(igraph::cut_at(walktrapClusters, n = j))))
    }
